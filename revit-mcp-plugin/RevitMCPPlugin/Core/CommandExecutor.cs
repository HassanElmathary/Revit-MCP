    ...
    public static class CommandExecutor
    {
        ...
        private static JToken ExportToDwg(Document doc, JObject parameters)
        {
            ...
            try
            {
                ...
                dwgOpts.MergedViews = true;    // Flatten linked models into one DWG
                dwgOpts.HideScopeBox = true;
                dwgOpts.HideReferencePlane = true;
                ...
                foreach (var vid in viewIds.Take(50))
                {
                    try
                    {
                        ...
                        // Snapshot existing files before export to identify extras
                        var filesBefore = new HashSet<string>(
                            System.IO.Directory.GetFiles(outputFolder),
                            StringComparer.OrdinalIgnoreCase);

                        doc.Export(outputFolder, cleanName, ids, dwgOpts);

                        // Clean up extra files generated by Revit (xrefs, PCP, images)
                        var mainDwg = System.IO.Path.Combine(outputFolder, cleanName + ".dwg");
                        var filesAfter = System.IO.Directory.GetFiles(outputFolder);
                        foreach (var file in filesAfter)
                        {
                            if (filesBefore.Contains(file)) continue; // existed before
                            if (string.Equals(file, mainDwg, StringComparison.OrdinalIgnoreCase)) continue; // keep main DWG
                            // Remove linked DWG xrefs, PCP plot configs, and raster debris
                            var ext = System.IO.Path.GetExtension(file).ToLower();
                            if (ext == ".pcp" || ext == ".tif" || ext == ".jpg" || ext == ".png")
                            {
                                try { System.IO.File.Delete(file); } catch { }
                            }
                            // Delete linked-model DWG xrefs (extra DWGs that are NOT the main file)
                            else if (ext == ".dwg" && !string.Equals(file, mainDwg, StringComparison.OrdinalIgnoreCase))
                            {
                                try { System.IO.File.Delete(file); } catch { }
                            }
                        }
